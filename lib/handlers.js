...
handlers._checks.post = (data, cb) => {
    ...
    successCode = typeof(successCode) == 'object' 
    && successCode instanceof Array && successCode.length > 0 ? successCode : false;
...
handlers._checks.put = (data, cb) => {
    ...
    console.log('====', protocol, timeoutSeconds, successCode, method, url, '====')

    if(id){
        if(protocol || timeoutSeconds || successCode || method || url){
            _data.read('checks', id, (err, checkData)=>{
                if(!err && checkData) {
                    const token = typeof (data.headers.token) == 'string' ? data.headers.token : false;
                    handlers._tokens.verifyToken(token, checkData.userPhone, (tokenIsValid)=>{
